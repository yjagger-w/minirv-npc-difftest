...
class GuestMemory {
 public:
  ...
  std::size_t load_binary(const std::string& path) {
    ...
    if (length <= 0 || static_cast<std::uint64_t>(length) > bytes_.size()) {
      throw std::runtime_error("raw binary is empty or exceeds guest memory");
    }
    input.seekg(0);
  ...
  std::uint32_t read_word(std::uint32_t address) const {
    ...
    for (unsigned byte = 0; byte < 4; ++byte) {
      value |= static_cast<std::uint32_t>(bytes_[offset + byte]) << (8U * byte);
    }
    return value;
  }
  ...
  void apply_store(std::uint32_t address, std::uint32_t data,
    ...
    for (unsigned byte = 0; byte < 4; ++byte) {
      if ((mask & (1U << byte)) != 0U) {
        bytes_[offset + byte] = static_cast<std::uint8_t>(data >> (8U * byte));
      }
    }
  }

  static bool contains(std::uint32_t address) {
    return address >= kMemoryBase && address <= 0x87ffffffU;
  }
 ...
 private:
  std::size_t translate(std::uint32_t address, std::size_t width,
    ...
    if (!contains(address)) {
      throw std::out_of_range(std::string(operation) + " outside guest RAM");
    }
    const std::uint64_t offset =
        static_cast<std::uint64_t>(address) - kMemoryBase;
    if (offset + width > bytes_.size()) {
      throw std::out_of_range(std::string(operation) + " beyond guest RAM");
    }
...
class Runner {
 public:
  Runner(const std::string& image, std::size_t cycle_limit, bool waveform)
    ...
    const std::size_t size = memory_.load_binary(image);
    std::cout << "SoC image: " << image << '\n'
              << "Image size: " << size << " bytes\n"
              << "Guest load address: 0x80000000\n"
              << "Cycle limit: " << cycle_limit_ << '\n';
    context_->traceEverOn(waveform);
    if (waveform) {
      trace_ = std::make_unique<VerilatedFstC>();
      dut_->trace(trace_.get(), 8);
      trace_->open("build/minirv_soc_am.fst");
    }
    dut_->clk = 0;
    dut_->reset = 1;
    dut_->gpio_input = 0;
  ...
  int run() {
    for (std::size_t cycle = 0; cycle < cycle_limit_; ++cycle) {
      if (dut_->bus_error) {
        std::cerr << "SoC unmapped data access at 0x" << std::hex
                  << dut_->commit_mem_addr << std::dec << '\n';
        return 2;
      }
      tick();
      if (dut_->uart_tx_valid) {
        std::cout << static_cast<char>(dut_->uart_tx_data) << std::flush;
      }
      if (dut_->commit_valid && dut_->commit_mem_wen &&
          GuestMemory::contains(dut_->commit_mem_addr)) {
        memory_.apply_store(dut_->commit_mem_addr, dut_->commit_mem_wdata,
                            static_cast<std::uint8_t>(dut_->commit_mem_wmask));
        settle();
      }
      if (dut_->trap_valid) {
        std::cerr << "SoC architectural trap: cause=0x" << std::hex
                  << static_cast<unsigned>(dut_->trap_cause) << std::dec << '\n';
        return 2;
      }
    ...
    std::cerr << "SoC timeout after " << cycle_limit_ << " cycles\n";
    return 3;
  }

 private:
  void drive_instruction() {
    dut_->imem_rdata = dut_->reset ? 0U : memory_.read_word(dut_->imem_addr);
  }

  void drive_ram() {
    if (!dut_->reset && dut_->ram_valid && !dut_->ram_write) {
      dut_->ram_rdata = memory_.read_word(dut_->ram_addr);
    } else {
      dut_->ram_rdata = 0;
    }
  }
  ...
  void settle(bool suppress_first_data = false) {
    for (int iteration = 0; iteration < 3; ++iteration) {
      drive_instruction();
      dut_->eval();
      if (!suppress_first_data || iteration != 0) drive_ram();
      dut_->eval();
    }
  }
...
bool parse_limit(const std::string& text, std::size_t& value) {
  std::uint64_t parsed = 0;
  const auto result = std::from_chars(text.data(), text.data() + text.size(),
                                      parsed, 10);
  if (text.empty() || result.ec != std::errc{} ||
      result.ptr != text.data() + text.size() || parsed == 0U ||
...
int main(int argc, char** argv) {
  bool waveform = false;
  std::string image;
  std::size_t limit = kDefaultCycleLimit;
  for (int index = 1; index < argc; ++index) {
    const std::string argument = argv[index];
    if (argument == "--fst") waveform = true;
    else if (argument == "--max-cycles" && index + 1 < argc) {
      if (!parse_limit(argv[++index], limit)) return 2;
    } else if (image.empty()) image = argument;
    else return 2;
  }
  if (image.empty()) return 2;
  try {
    Runner runner(image, limit, waveform);
    return runner.run();
  } catch (const std::exception& error) {
    std::cerr << "SoC runner error: " << error.what() << '\n';
    return 2;
  }
}
